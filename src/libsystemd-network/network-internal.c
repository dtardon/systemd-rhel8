...
int net_get_unique_predictable_data(struct udev_device *device, bool use_sysname, uint64_t *result) {
        ...
        if (!name && use_sysname)
                (void) sd_device_get_sysname(device->device, &name);
        if (!name)
                return log_device_debug_errno(device->device, ENODATA,
                                              "No stable identifying information found");

        log_device_debug(device->device, "Using \"%s\" as stable identifying information", name);
        l = strlen(name);
