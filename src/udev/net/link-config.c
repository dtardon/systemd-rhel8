        ...
        LIST_FOREACH(links, link, ctx->links) {
                ...
                if (net_match_config(link->match_mac, link->match_path, link->match_driver,
                        ...
                        if (link->match_name) {
                                ...
                                (void) link_unsigned_attribute(device->device, "name_assign_type", &name_assign_type);
...
static int get_mac(struct udev_device *device, MACPolicy policy, struct ether_addr *mac) {
        unsigned addr_type;
        ...
        r = link_unsigned_attribute(device->device, "addr_assign_type", &addr_type);
        if (r < 0)
                return r;
        switch (addr_type) {
        case NET_ADDR_SET:
                return log_device_debug(device->device, "MAC on the device already set by userspace");
        case NET_ADDR_STOLEN:
                return log_device_debug(device->device, "MAC on the device already set based on another device");
        case NET_ADDR_RANDOM:
        case NET_ADDR_PERM:
                break;
        default:
                return log_device_warning(device->device, "Unknown addr_assign_type %u, ignoring", addr_type);
        }

        if (want_random == (addr_type == NET_ADDR_RANDOM))
                return log_device_debug(device->device, "MAC on the device already matches policy *%s*",
                                        mac_policy_to_string(policy));

        if (want_random) {
                log_device_debug(device->device, "Using random bytes to generate MAC");
...
int link_config_apply(link_config_ctx *ctx, link_config *config,
        ...
        (void) link_unsigned_attribute(device->device, "name_assign_type", &name_type);
